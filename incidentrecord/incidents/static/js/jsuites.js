...
jSuites.chart = (function(el, options) {
    var obj = {};
    obj.options = {};
    obj.values = [];
    ...
    var palette = {
        'jungle': [ '#dea394', '#fceee9', '#dde6d5', '#a3b899', '#667b68'],
    };
     ...
     * Get total base
     */
    obj.total = function(arr) {
        return arr.reduce((a,b) => a + b, 0);
    }

    /**
     * Get color hex by index number
     */
    obj.getColor = function(index) {
        // Colors
        var colors = palette[obj.options.palette];

        // Make sure index within number of available colours in the palette
        if (! colors[index]) {
     ...
     * Create labels
     */
    obj.labels = function(labels) {
        var labelsContainer = document.createElement('div');
        labelsContainer.className = 'jchart-labels';
        ...
        for (var i = 0; i < obj.options.data.length; i++) {
            ...
            divColor.style.backgroundColor = obj.getColor(i);
            divColor.className = 'jchart-label-color';
            div.appendChild(divColor);

            var divLabel = document.createElement('div');
            divLabel.className = 'jchart-label';
            divLabel.innerHTML = labels[i];
    ...
    obj.pie = function() {
        ...
        el.classList.add('jchart-pie');

        // Create sections
        var total = obj.total(obj.options.data);
        var position = 0;

        for (var i = 0; i < obj.options.data.length; i++) {
            var value = (obj.options.data[i] / total) * 100;
            var angle = position + ((360 * value) / 100);
            path[i] = document.createElementNS(xmlns, "path");
            path[i].setAttribute('fill', obj.getColor(i));
            path[i].setAttribute('d', arc(50, 50, 50, position, angle));
        ...
        // Create labels
        if (obj.options.labels.length) {
            obj.labels(obj.options.labels);
        }
    }

    //<g><path class="line" fill="transparent" stroke="red" vector-effect="non-scaling-stroke" d=" M 6 6 L 52 22 L 98 12 L 144 25"></path></g>

    obj.dots = function() {
        // Create bars
        var num = 100 / obj.options.data.length;
        var total = obj.total(obj.options.data);
        var position = 0;
        var color = 0;
        ...
        for (var i = 0; i < obj.options.data.length; i++) {
            var value = (obj.options.data[i] / total) * 100;
            path[i] = document.createElementNS(xmlns, "circle");
            path[i].setAttribute('fill', obj.getColor(i));
            path[i].setAttribute('cx', 10 + (i * num));
            path[i].setAttribute('cy', value);
            path[i].setAttribute('r', 2);
            g.appendChild(path[i]);
        }
    ...
    obj.meter = function() {
        ...
        var angle = Math.round(180 - (180 * value / 100));
        var x =  50 + (50 * (Math.cos(angle *  Math.PI/180)));
        var y =  50 - (50 * Math.sin(angle *  Math.PI/180));
        // Create container
        var g = document.createElementNS(xmlns, "g");
        // Create semi circle
        path[0] = document.createElementNS(xmlns, "path");
        path[0].setAttribute('fill', '#efefef');
        path[0].setAttribute('d', arc(50, 50, 50, 270, 90));
        g.appendChild(path[0]);
        // Create semi circle
        path[1] = document.createElementNS(xmlns, "path");
        path[1].setAttribute('fill', obj.getColor(0));
        path[1].setAttribute('d', arc(50, 50, 50, 270, 310));
        g.appendChild(path[1]);
        // Create semi circle
        path[2] = document.createElementNS(xmlns, "path");
        path[2].setAttribute('fill', '#ffffff');
        path[2].setAttribute('d', arc(50, 50, 40, 270, 90));
        g.appendChild(path[2]);
        // Append data
        svg.appendChild(g);
        // Line
        //<line id="pointer" x1="14.64" y1="14.64" x2="50" y2="50" style="stroke: rgb(176, 176, 176); stroke-width: 2;"></line>
        // Create semi circle
        ...
        // Create semi circle
        path[3] = document.createElementNS(xmlns, "path");
        path[3].setAttribute('fill', '#ffffff');
        path[3].setAttribute('d', arc(50, 50, 10, 270, 90));
        svg.appendChild(path[3]);
    }

    if (obj.options.type == 'pie') {
        obj.pie();
    } else if (obj.options.type == 'doughnut') {

    } else if (obj.options.type == 'dots') {
        obj.dots();
    } else if (obj.options.type == 'meter') {
        obj.meter();
    }
    ...
    if (obj.options.title) {
        ...
        title.innerHTML = obj.options.title;
        title.classList.add('jchart-title');
        el.appendChild(title);
    }
    el.classList.add('jchart');
